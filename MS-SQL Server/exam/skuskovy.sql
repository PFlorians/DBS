use skuskova
--set compatibility to SQL Server 2005, owner to [sa] if not by default to be able to generate DBS diagrams
-- Vytvoreni schemat
go
...
select * from [tempdb].INFORMATION_SCHEMA.TABLES
	...
	select * from sys.columns as sc join sys.tables as st on st.object_id=sc.object_id 

	select sc.name, sc.max_length ,sst.name, sst.system_type_id from sys.columns as sc
		inner join sys.tables as st on st.object_id=sc.object_id 
		inner join sys.types as sst on sst.system_type_id=sc.system_type_id
		inner join sys.schemas as ss on ss.schema_id=st.schema_id
	where st.name like 'Zbozi' and ss.name like 'Zbozi';
go
create trigger zaloha_tmp
	...
	declare @nazev as varchar(60);
	declare @cena as money;
	declare @predaj as money;
	declare @sklad as int;

	if('zbozi_tmp' in(select name from [tempdb].sys.tables))
	begin
		insert into #zbozi_tmp(Dph, Marze, ZboziID, ZboziKategorie, Nasklade, CenazaJednotku, ProdejniCenazaJ, NazevZbozi,MernaJednotka,ZboziPopis)
			(select d.Dph, d.Marze, d.ZboziID, d.ZboziKategorie, d.Nasklade, d.CenazaJednotku, d.ProdejniCenazaJ, d.NazevZbozi, d.MernaJednotka, d.ZboziPopis from deleted as d);
			declare kur cursor scroll for select NazevZbozi, CenazaJednotku, ProdejniCenazaJ, Nasklade from #zbozi_tmp;
			open kur;
			fetch first from kur into @nazev, @cena, @predaj, @sklad;
			while @@FETCH_STATUS = 0
			begin
				select @nazev + ' ' + convert(varchar(8), @cena) + ' ' + convert(varchar(8), @predaj) + ' ' + convert(varchar(8), @sklad) as [info];
				fetch next from kur into @nazev, @cena, @predaj, @sklad;
			end; 
			close kur;
			deallocate kur;
	end;
	...
	begin
		create table #zbozi_tmp(
			...
			ZboziKategorie int,
			Nasklade int,
			CenazaJednotku money,
			ProdejniCenazaJ money,
			NazevZbozi varchar(60),
			MernaJednotka varchar(2),
			ZboziPopis varchar(250)
		);
		insert into #zbozi_tmp(Dph, Marze, ZboziID, ZboziKategorie, Nasklade, CenazaJednotku, ProdejniCenazaJ, NazevZbozi,MernaJednotka,ZboziPopis)
		(select d.Dph, d.Marze, d.ZboziID, d.ZboziKategorie, d.Nasklade, d.CenazaJednotku, d.ProdejniCenazaJ, d.NazevZbozi, d.MernaJednotka, d.ZboziPopis from deleted as d);
		declare kur cursor scroll for select NazevZbozi, CenazaJednotku, ProdejniCenazaJ, Nasklade from #zbozi_tmp;
...
begin tran t5
	delete from Objednavka.Polozky where ZboziID = 28;
	delete from Dodavatel.Zbozi where ZboziID = 28;
	delete from Zbozi.Zbozi where ZboziID = 28;
	select * from [tempdb].sys.tables;
	select * from Zbozi.Zbozi
	select * from #zbozi_tmp;
rollback tran t5
 drop database skuskova 
