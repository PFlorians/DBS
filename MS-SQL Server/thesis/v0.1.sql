		...
		begin
			insert into attendance.summary_bonuses(bonus_id, summary_id, day, bonus_hours)
				select (case 
						...
						(case
							when @lastShift like 'O%' then (@hours_worked_day*(select top 1 [% bonus]/100 
																				from attendance.bonus
																				where bonus_id like '0110'))
							when @lastShift like 'N%' then (@hours_worked_day*(select top 1 [% bonus]/100 
																				from attendance.bonus
																				where bonus_id like '0123'))
							when ((DATEPART(weekday, convert(date, @lastDate, 101)) = 6) or 
								(DATEPART(weekday, convert(date, @lastDate, 101)) = 7)) then 
															(@hours_worked_day*(select top 1 [% bonus]/100 
																				from attendance.bonus
																				where bonus_id like '0130'))
							when (convert(date, @lastDate, 101) in (select [date] from attendance.public_holidays))
								then (@hours_worked_day*(select top 1 [% bonus]/100 
														from attendance.bonus
	...
	begin try
		...
		begin
			...
			begin
				...
				exec determineOvertime @hours_worked_day, @expectedWorkTime, @lastShift, @lastDate, @summaryCreated, @errMsg=@errMsg;
				if(@errMsg is not null)
				begin
			...
			if(@errMsg is not null)
			begin
				print 'Error determining absence: ' + @errMsg;
			end;
		...
		begin
			...
			if(@errMsg is not null)
			begin
				print 'Error determining absence 2: ' + @errMsg;
			end;
		end;
	end try
	begin catch
